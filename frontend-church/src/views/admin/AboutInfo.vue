    ...
    <div class="admin-about-info">
      ...
      <div class="about-info-list">
        <div v-if="filteredList.length === 0" class="empty-state">
          <p>{{ aboutInfoList.length === 0 ? '尚無關於我們資料' : '沒有符合條件的資料' }}</p>
        </div>
        <div v-else class="info-table">
          <div class="table-header">
            <h3>關於我們列表 (共 {{ filteredList.length }} 筆)</h3>
          </div>
          <table>
            <thead>
              <tr>
...
const filteredList = computed(() => {
  let filtered = [...aboutInfoList.value]
  
  if (filters.value.sectionKey) {
    filtered = filtered.filter(info => 
      info.sectionKey?.toLowerCase().includes(filters.value.sectionKey.toLowerCase())
    )
  }
  
  if (filters.value.title) {
    filtered = filtered.filter(info => 
      info.title?.toLowerCase().includes(filters.value.title.toLowerCase())
    )
  }
...
const loadAboutInfo = async () => {
  try {
    const response = await apiRequest('/church/admin/about-info', {
      method: 'GET',
      credentials: 'include'
    })
    ...
    alert('載入關於我們資訊失敗: ' + error.message)
  }
}
...
const saveInfo = async () => {
  try {
    ...
    alert('儲存關於我們資訊失敗: ' + error.message)
  }
}
...
const deleteInfo = async (id) => {
  ...
  try {
    ...
    alert('刪除關於我們資訊失敗: ' + error.message)
  }
}
