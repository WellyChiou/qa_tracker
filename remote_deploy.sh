...
EOF
    ...
    echo "*/5 * * * * cd $PROJECT_DIR && $PROJECT_DIR/monitor-frontend.sh >> $LOGS_DIR/frontend-monitor_\$(date +\%Y\%m\%d).log 2>&1" >> "$TEMP_CRON"
    
    # 設置系統資源監控（每小時）
    echo "✅ 設置系統資源監控（每小時檢查一次）..."
    echo "0 * * * * cd $PROJECT_DIR && $PROJECT_DIR/monitor-system.sh >> $LOGS_DIR/system-monitor_\$(date +\%Y\%m\%d).log 2>&1" >> "$TEMP_CRON"
    ...
    echo "0 3 * * * find $LOGS_DIR -name \"*.log\" -type f -mtime +7 -delete" >> "$TEMP_CRON"
