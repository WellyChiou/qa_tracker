      ...
      <form @submit.prevent="handleSubmit" class="modal-body">
        <div class="form-group">
          ...
          <input
            ...
            type="text"
            required
            placeholder="例如: /api/personal/menus/frontend 或 /api/personal/positions/*/persons"
          />
          <small class="form-hint">支持通配符：* 表示單層路徑（如 /api/personal/positions/*/persons），** 表示多層路徑（如 /api/personal/positions/**）</small>
        </div>
...
const handleSubmit = async () => {
  ...
  try {
    const url = isEdit.value 
      ? `/url-permissions/${props.permission.id}`
      : '/url-permissions'
