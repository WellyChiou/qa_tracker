  ...
  <div class="admin-page">
    ...
    <header class="header">
      <div class="header-top">
        <h1>🔗 URL 權限管理</h1>
        <button class="btn btn-primary" @click="showAddModal = true">新增 URL 權限</button>
      </div>
    </header>
    ...
    <div v-if="showAddModal || editingPermission" class="modal-overlay" @click="closeModal">
      <div class="modal-content" @click.stop>
        <h2>{{ editingPermission ? '編輯 URL 權限' : '新增 URL 權限' }}</h2>
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            <label>URL 模式</label>
            <input v-model="form.urlPattern" placeholder="/api/**" required />
          </div>
          <div class="form-group">
            <label>HTTP 方法</label>
          ...
          <div class="form-group">
            <label>所需角色</label>
            <input v-model="form.requiredRole" placeholder="ROLE_ADMIN" />
          </div>
          <div class="form-group">
            <label>所需權限</label>
            <input v-model="form.requiredPermission" placeholder="PERMISSION_CODE" />
          </div>
          <div class="form-group">
            <label>公開（無需認證）</label>
