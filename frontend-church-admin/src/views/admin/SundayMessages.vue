  ...
  <AdminLayout>
    <div class="admin-sunday-messages">
      <div class="page-header">
        <div>
          <h2>主日信息管理</h2>
          <p>維護主日講題、講員與 DM 圖片，對應公開站台主日信息內容。</p>
        </div>
      ...
      <section class="overview-strip">
        ...
        <article class="overview-card">
          <span>當前頁面</span>
          <strong>{{ messagesList.length }}</strong>
          <p>這一頁實際載入的主日信息資料。</p>
        </article>
        <article class="overview-card">
          <span>查詢狀態</span>
          <strong>{{ filters.title || filters.startDate || filters.endDate || filters.serviceType || filters.isActive !== '' ? '已套用' : '全部' }}</strong>
          <p>可透過日期、聚會時間、標題與狀態快速查找信息。</p>
        </article>
      </section>
      ...
      <div class="messages-list card surface-card">
