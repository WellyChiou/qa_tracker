...
public class PersonalJwtUtil {
    ...
    private SecretKey getSigningKey() {
        // 從資料庫讀取最新的 secret
        String currentSecret = systemSettingService.getSettingValue("jwt.secret", null);
        if (currentSecret == null || currentSecret.isEmpty()) {
            currentSecret = "F/cPluFKK3/44X5iX9GdY6P7Ye+BIDdBTw6uljBTl9o="; // Default backup secret
        }
    ...
    private Long getAccessTokenExpiration() {
        return systemSettingService.getSettingValueAsInt("jwt.access-token-expiration", 3600000) * 1L;
    }

    private Long getRefreshTokenExpiration() {
        return systemSettingService.getSettingValueAsInt("jwt.refresh-token-expiration", 604800000) * 1L;
    }

    private Boolean isRefreshTokenEnabled() {
        return systemSettingService.getSettingValueAsBoolean("jwt.refresh-token-enabled", true);
    }
